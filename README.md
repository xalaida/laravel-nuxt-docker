...
## Stack includes
* Laravel (^6.0) (fresh clean version)
* Nuxt.JS (^2.0) (fresh clean version)
* PostgreSQL 11.3
* Redis 3.0
...
_It lasts for 10 minutes, so you can take a coffee break_
...
If you want to update dependencies, you should restart nuxt process, which will be automatically restarted with node container, so run
```
...
```
If you want to generate a new controller, a command should be executed from the current user like this:
```
...
Install laravel from the container working directory (our just created api directory on the previous step)
```
cd api
